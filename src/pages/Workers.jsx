      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          {selectedWorkers.length > 0 && !guestMode && (
            <button onClick={handleDeleteSelected} className="bg-red-500 text-white px-4 py-2.5 rounded-lg hover:bg-red-600 transition-colors font-medium text-sm">
              Delete Selected ({selectedWorkers.length})
            </button>
          )}
          {!guestMode && (
            <button onClick={handleDeleteAll} className="bg-red-600 text-white px-4 py-2.5 rounded-lg hover:bg-red-700 transition-colors font-medium text-sm flex items-center gap-1">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
              Delete All
            </button>
          )}
          {!guestMode && (
            <button onClick={() => { resetForm(); setShowForm(true); }} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm flex items-center gap-1">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
              Add Worker
            </button>
          )}
          {!guestMode && (
            <button onClick={() => setShowTrash(!showTrash)} className={`px-4 py-2.5 rounded-lg font-medium text-sm transition-colors ${showTrash ? 'bg-orange-600 text-white' : 'bg-orange-100 text-orange-700 hover:bg-orange-200'}`}>
      ...
      {showConfirmDialog && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-xl shadow-2xl p-4 sm:p-6 w-full max-w-md">
            <h2 className="text-xl font-bold text-gray-800 mb-2">{showConfirmDialog.title}</h2>
            <p className="text-gray-600 mb-6">{showConfirmDialog.message}</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowConfirmDialog(null)} className="px-4 py-2 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 font-medium">Cancel</button>
              <button onClick={showConfirmDialog.onConfirm} className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-medium">Delete</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex gap-3 justify-end">
                <button type="button" onClick={resetForm} className="px-4 py-2 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 font-medium">Cancel</button>
                <button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium">
                  {editingWorker ? 'Save Worker' : 'Save Worker'}
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="p-4 sm:p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <p className="text-lg font-semibold text-gray-800">{showProfile.name}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Mobile Number</p>
                  <p className="text-lg font-semibold text-gray-800">{showProfile.mobile}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Joining Date</p>
                  <p className="text-lg font-semibold text-gray-800">{showProfile.joiningDate || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Status</p>
                  <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${showProfile.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              ...
              <div className="flex gap-1 sm:gap-2 mb-4 border-b border-gray-200 pb-2 overflow-x-auto">
                {['salary', 'attendance', 'advance'].map(tab => (
                  <button
                    ...
                    className={`px-2 sm:px-4 py-2 rounded-t-lg text-xs sm:text-sm font-medium transition-colors whitespace-nowrap ${profileTab === tab ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                ...
                {!guestMode && (
                  <button
                    onClick={() => {
                      exportWorkerPDF(showProfile, salaries);
                    }}
                    className="ml-auto px-2 sm:px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-xs sm:text-sm font-medium whitespace-nowrap"
                  >
                    Export PDF
                  </button>
                )}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        ...
                        <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Working Days</th>
                        <th className="px-4 py-3 text-right text-sm font-semibold text-gray-600">Total Salary</th>
                        <th className="px-4 py-3 text-right text-sm font-semibold text-gray-600">Advance</th>
                        <th className="px-4 py-3 text-right text-sm font-semibold text-gray-600">Final Salary</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {getWorkerSalaries(showProfile.id).map((s, i) => (
                        <tr key={s.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900">{s.workingDays}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 text-right">₹{Number(s.totalSalary).toLocaleString('en-IN')}</td>
                          <td className="px-4 py-3 text-sm text-orange-600 text-right">₹{Number(s.advance).toLocaleString('en-IN')}</td>
                          <td className="px-4 py-3 text-sm text-green-700 text-right font-bold">₹{Number(s.finalSalary).toLocaleString('en-IN')}</td>
                        </tr>
                      ))}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        ...
                        <th className="px-4 py-3 text-center text-sm font-semibold text-gray-600">Working Days</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {getWorkerAttendance(showProfile.id).map((a, i) => (
                        <tr key={i} className="hover:bg-gray-50">
                          <td className="px-4 py-3 text-sm text-gray-500">{i + 1}</td>
                          <td className="px-4 py-3 text-sm text-gray-900">{a.month}</td>
                          <td className="px-4 py-3 text-sm text-gray-600 text-center">{a.days}</td>
                        </tr>
                      ))}
                      {getWorkerAttendance(showProfile.id).length === 0 && (
                        <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400">No attendance records found.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200">
                        <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">#</th>
                        <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Month</th>
                        <th className="px-4 py-3 text-right text-sm font-semibold text-gray-600">Advance Amount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {getWorkerSalaries(showProfile.id).filter(s => Number(s.advance) > 0).map((s, i) => (
                        <tr key={s.id} className="hover:bg-gray-50">
                          <td className="px-4 py-3 text-sm text-gray-500">{i + 1}</td>
                          <td className="px-4 py-3 text-sm text-gray-900">{s.createdAt ? new Date(s.createdAt).toLocaleString('default', { month: 'short', year: 'numeric' }) : 'N/A'}</td>
                          <td className="px-4 py-3 text-sm text-orange-600 text-right font-medium">₹{Number(s.advance).toLocaleString('en-IN')}</td>
                        </tr>
                      ))}
                      {getWorkerSalaries(showProfile.id).filter(s => Number(s.advance) > 0).length === 0 && (
                        <tr><td colSpan={3} className="px-4 py-8 text-center text-gray-400">No advance records found.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                {!guestMode && (
                  <th className="px-2 py-3 sm:px-4 sm:py-4 w-10">
                ...
                <th className="px-2 py-3 sm:px-4 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider">#</th>
                <th className="px-2 py-3 sm:px-4 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider">Name</th>
                <th className="px-2 py-3 sm:px-4 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider hidden sm:table-cell">Mobile</th>
                <th className="px-2 py-3 sm:px-4 sm:py-4 text-left text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider hidden sm:table-cell">Status</th>
                <th className="px-2 py-3 sm:px-4 sm:py-4 text-right text-xs sm:text-sm font-semibold text-gray-600 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {filteredWorkers.length === 0 ? (
                ...
                filteredWorkers.map((worker, index) => (
                  <tr key={worker.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-2 py-3 sm:px-4 sm:py-4 text-xs sm:text-sm text-gray-500">{index + 1}</td>
                    <td className="px-2 py-3 sm:px-4 sm:py-4 text-xs sm:text-sm font-medium text-gray-900">{worker.name}</td>
                    <td className="px-2 py-3 sm:px-4 sm:py-4 text-xs sm:text-sm text-gray-600 hidden sm:table-cell">{worker.mobile}</td>
                    ...
                    <td className="px-2 py-3 sm:px-4 sm:py-4 text-right whitespace-nowrap">
                      <button onClick={() => handleViewProfile(worker)} className="text-purple-600 hover:text-purple-800 font-medium text-xs sm:text-sm mr-1 sm:mr-2">View</button>
                      {!guestMode && (
                        <>
                          <button onClick={() => handleEdit(worker)} className="text-blue-600 hover:text-blue-800 font-medium text-xs sm:text-sm mr-1 sm:mr-2">Edit</button>
                          <button onClick={() => handleDelete(worker.id, worker.name)} className="text-red-600 hover:text-red-800 font-medium text-xs sm:text-sm">Delete</button>
                        </>
                      )}
                    </td>
                  </tr>
